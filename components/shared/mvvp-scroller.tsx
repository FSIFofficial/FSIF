...
export function MvvpScroller({ items, theme = 'dark', eyebrow = 'PHILOSOPHY / 私たちの理念' }: MvvpScrollerProps) {
  ...
  // scroll — critical here, since on the homepage this section sits far
    ...
    <section aria-label="FSIFの理念（MVVP）" className={cn('relative', t.section)}>
      ...
      <div className={cn('sticky z-30 border-b backdrop-blur lg:hidden', t.tabBar)} style={{ top: 'var(--header-h)' }}>
        <div className="container-fsif py-3">
          ...
          <div ref={tabListRef} className="flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="理念">
            {items.map((p, i) => (
              <button
                key={p.key}
                ref={(el) => {
