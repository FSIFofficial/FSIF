...
const researchMedia = media.filter((m) => ['Research', 'Report'].includes(m.category)).slice(0, 3)
  ...
  return (
    <>
      <PageHero
        ...
        description="宇宙産業の動向を客観的に捉え、レポートや提言として発信します。MEDIAと連携し、知を開いていきます。"
      ...
      <section className="bg-background py-16 md:py-20">
        <div className="container-fsif">
          <div className="flex items-end justify-between gap-4">
            <SectionHeading labelEn="LATEST RESEARCH" title="最新のレポート" />
            <CtaLink href="/media" variant="text" className="pb-1">MEDIAで読む</CtaLink>
          </div>
          <ul className="mt-8 divide-y divide-border border-y border-border">
            {researchMedia.map((m) => (
              <li key={m.slug}>
                <Link href={`/media/${m.slug}`} className="group flex items-center gap-4 py-5">
                  <span className="inline-flex items-center rounded-sm border border-fsif-blue px-2 py-1 font-mono text-[0.65rem] tracking-wide text-fsif-blue">
                    {m.category}
                  </span>
                  <div className="min-w-0 flex-1">
                    <time className="font-mono text-xs text-muted-foreground">{formatDate(m.date)}</time>
                    <h3 className="mt-0.5 text-pretty font-bold text-foreground transition-colors group-hover:text-fsif-blue">
                      {m.title}
                    </h3>
                  </div>
